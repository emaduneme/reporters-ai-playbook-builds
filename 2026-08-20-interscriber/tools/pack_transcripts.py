...
def format_time(seconds: float) -> str:
    """Format a time in seconds as an intuitive MM:SS.xx or HH:MM:SS.xx."""
    h = int(seconds // 3600)
...
def render_markdown(entries: list[tuple[str, float, list[dict]]], silence_threshold: float) -> str:
    ...
    for name, duration, phrases in entries:
        ...
        # Only show speaker labels when more than one speaker is present
        speaker_ids = {p.get("speaker_id") for p in phrases if p.get("speaker_id") is not None}
        multi_speaker = len(speaker_ids) > 1

        for p in phrases:
            spk = p.get("speaker_id")
            if multi_speaker and spk is not None:
                spk_str = str(spk)
                if spk_str.startswith("speaker_"):
                    try:
                        spk_num = int(spk_str[len("speaker_"):]) + 1
                    except ValueError:
                        spk_num = spk_str
                else:
                    spk_num = spk_str
                spk_tag = f" **Speaker {spk_num}:**"
            else:
                spk_tag = ""
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        nargs="+",
        default=None,
        help="Specific transcript JSON files to pack (default: all *.json in transcripts/)",
    )
    args = ap.parse_args()

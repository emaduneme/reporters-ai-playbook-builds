...
- **Who it's for:** any reporter who transcribes interviews and is tired of a subscription they don't fully use — beginner lane (just run it), with the code open for anyone on the deploy lane who wants to modify it.
...
## Recreate it with Claude Cowork (no terminal)

If you've never touched a terminal and don't want to start now, Claude Cowork is the better entry point — it's a paid-plan feature (Pro/Max/Team/Enterprise) in the Claude desktop app, web, or mobile that reads and writes your local files without you running any commands yourself. Worth knowing before you try it: Cowork executes code in an isolated environment on Anthropic's servers, not literally on your laptop, so it can't just "run our Streamlit app" the way Claude Code can — instead, describe the outcome and let it build its own path there. A prompt to paste into a Cowork task:
...
Two honest tradeoffs versus the local route: Cowork's transcription happens via a cloud API call (Groq), not fully on-device, so it doesn't carry the same "audio never leaves your machine" privacy guarantee as InterScriber's local Whisper mode — and it requires a paid Claude plan. If source protection is the priority, use Claude Code with local Whisper instead.
